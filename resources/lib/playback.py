    ...
    def parse_detail(self, details, cdn):
        for i in details:
            if cdn == self.clean_name([i['CdnName']])[0] or not cdn:
                headers = self.plugin.get_basic_request_headers().copy()
                manifestUrl = None
                if i.get('DaiVod') and i.get('DaiVod').get('ContentSourceId') and i.get('DaiVod').get('VideoId'):
                    ...
                    res = self.requests.exchange(url, headers=headers, method='POST')
                    if res.status == 201:
                    ...
                    res = self.requests.exchange(url, headers=headers, fields=data)
                    if res.status == 201:
                        manifestUrl = res.json().get('stream_manifest')
                if manifestUrl is None:
                    ...
                    res = self.requests.exchange(url, headers=headers, method='HEAD')
